...
<?php 
	...
	class admin
	{
		...
		public function insert_Admin($data){
			...
			$password2 = mysqli_real_escape_string($this->db->link, ($data['confirmpassword']));
			...
			if($name == "" || $username == "" ||  $password2 == ""|| $password == "" || $email == ""){
				...
				{
					$alert = "<span>Mật khẩu không khớp :( Nhập lại</span>";
						return $alert;
				}
		...
		public function update_admin($data,$user){
			...
			//$repeat_password = mysqli_real_escape_string($this->db->link, $data['newpassword']);
				...
				$query="UPDATE tbl_admin SET admin_Pass=('$new_password'),admin_User='$username',admin_Name='$name' WHERE admin_User = '$user'";
		...
		public function reset_Password($user){
			$query = "UPDATE tbl_admin SET admin_Pass = '1111' WHERE admin_User = '$user'";
			$result = $this->db->update($query);
